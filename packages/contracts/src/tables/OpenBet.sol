...
uint256 constant _tableId = uint256(bytes32(abi.encodePacked(bytes16(""), bytes16("OpenBet"))));
uint256 constant OpenBetTableId = _tableId;
...
library OpenBet {
  ...
  /** Get open */
  function get() internal view returns (uint256 open) {
  ...
  function encode(uint256 open) internal view returns (bytes memory) {
    return abi.encodePacked(open);
  }
